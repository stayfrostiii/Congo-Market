...
def add_friend_to_account(db: Session, friend: FriendModel, request: Request):
    ...
    user_id = 1219588 #example hard-coded id number
    try:
...
def remove_friend_from_account(db: Session, friend: FriendModel, request: Request):
    # Simulate a user ID, you can replace this with an actual user ID from your database
    #user_id = request.cookies.get("user_id")
    user_id = 1219588  # example hard-coded id number
    try:
        ...
        # Check if friend with the same ID number exists in the friends_list
        ...
        friends = account.friends_list.split(",")
        friends.remove(friend.idNumber)  # Remove the friend
        account.friends_list = ",".join(friends)
